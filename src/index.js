...
class BitverseWeb3Provider extends EventEmitter {
  constructor(config) {
    super();
  ...
  setAddress(address) {
    ...
    for (var i = 0; i < window.frames.length; i++) {
      ...
      if (frame.ethereum && frame.ethereum.isBitverse) {
  ...
  request(payload) {
    ...
    var that = this;
    if (!(this instanceof BitverseWeb3Provider)) {
    ...
    switch (payload.method) {
      ...
      default:
        throw new ProviderRpcError(
          4200,
          `Bitverse does not support calling ${payload.method} synchronously without a callback. Please provide a callback parameter to call ${payload.method} asynchronously.`
        );
    }
    return response;
  }
    ...
    return new Promise((resolve, reject) => {
      ...
      switch (payload.method) {
        ...
        case 'eth_subscribe':
          throw new ProviderRpcError(
            4200,
            `Bitverse does not support calling ${payload.method}. Please use your own solution`
          );
        default:
          // call upstream rpc
  ...
  postMessage(handler, id, data) {
    if (this.ready || handler === 'requestAccounts') {
      ...
      if (window.bitversewallet.postMessage) {
        window.bitversewallet.postMessage(object);
      } else {
        // old clients
